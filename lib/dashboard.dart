...
class _DashboardState extends State<Dashboard> with SingleTickerProviderStateMixin{
  ...
  String? upcoming;String? canceled;String? ongoing;String? completed;
    ...
    return WillPopScope(
        ...
        child:Scaffold(
          appBar: appBar(context,[IconButton(onPressed: (){
            ...
            }, icon:Icon(Icons.sort,color:Theme.of(context).primaryColorDark ,))], now,null),
          body: SafeArea(
            top:true,
            child: SizedBox(width: deviceSize.width,height: deviceSize.height,
              child:  SingleChildScrollView(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    username.isNotEmpty? Container(width: deviceSize.width,height: deviceSize.height*0.08,
                           padding: const EdgeInsets.all(15),
                           child:Text('${greetingMessage()} ,  $username',
                             style:const TextStyle(fontSize: 20,fontWeight: FontWeight.bold),)):
                           Container(width: deviceSize.width,height: deviceSize.height*0.15,padding: const EdgeInsets.all(15),
                             child: InkWell(
                               onTap: (){Navigator.of(context).push(MaterialPageRoute(builder: (context)=>const UserProfile()));},
                               child: Row(
                                 children: [
                                   FadeTransition(opacity: controller,
                                   child: Text('addProfile'.tr,style:const TextStyle(fontSize: 22,fontWeight: FontWeight.bold))),
                                  // SizedBox(width: deviceSize.width*0.08,),
                                   SizedBox(width: deviceSize.width*0.22,
                                       child: Text(textAlign: TextAlign.end,'tap'.tr,style:const TextStyle(color: Colors.grey,fontSize: 14),))
                                 ],
                               ),
                             ),
                           ),
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        ...
                                        process(deviceSize,'Upcoming'.tr,const Color(0xfff7b17c),const Color(0xfffcdec7),Icons.timeline_outlined,upcomingProjects),
                                      ],
                                    ),
                                    Row(
                                      children: [
                                        process(deviceSize,'Completed'.tr,const Color(0xff7cc76d),const Color(0xffb9faac),Icons.check,completedProjects),
                                        process(deviceSize,'Canceled'.tr,const Color(0xffff7a92),const Color(0xfffaacba),Icons.clear,canceledProjects),
                                      ],
                                    ),
                                  ],
                                ),
                        ...
                        return Container(width: deviceSize.width,
                          ...
                          child: Container(height:70,decoration: BoxDecoration(color: Theme.of(context).primaryColor,borderRadius: BorderRadius.circular(12)),
                            child: ListTile(onTap: (){
                              Navigator.of(context).push(MaterialPageRoute(builder: (context)=>ProjectDetail(object: projectItem[index])));
                            },
                            leading: Container(decoration: BoxDecoration(border: Border.all(
                              ...
                              subtitle: LinearPercentIndicator( //leaner progress bar
                                ...
                                progressColor:dataModel.percentage!<100? const Color(0xffffb2a6):Colors.green,
                                barRadius: const Radius.circular(10),
                                backgroundColor: Colors.grey[300],
                              ),
                              trailing:sheet(deviceSize, dataModel.title, index)
                      ),
                          )
  ...
  process(dynamic deviceSize,String text,Color containerColor,Color bubbleColor,IconData icon,List object){
    return InkWell(onTap: (){
      Navigator.of(context).push(MaterialPageRoute(builder: (context)=>ProcessDetail(title: text,object: object,)));
    },
      child: Padding(
        ...
        child: Stack(alignment: Alignment.bottomRight,
          children: [
            Stack(alignment: Alignment.bottomLeft,
              children: [
                Container(width:deviceSize.width*0.444,height: deviceSize.height*0.10,
                  decoration: BoxDecoration(color: containerColor,borderRadius: BorderRadius.circular(15)),
                  ),
                Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: Text(text,textAlign: TextAlign.end,style:const TextStyle(fontSize: 20,color: Colors.white),),
                )
              ],
            ),
            Padding(
              padding: const EdgeInsets.all(8.0),
              child: CircleAvatar(backgroundColor:bubbleColor,radius:17,
...
sheet(dynamic deviceSize,dynamic title,int index){
    ...
    itemBuilder: (context) => [
      PopupMenuItem(
        ...
        child: InkWell(
          onTap: (){
            ...
            Navigator.of(context).push(MaterialPageRoute(builder: (context)=>EditTask(object: projectItem[index])));
          },
          child: Row(
            children:const [
              Icon(Icons.edit),
      ...
      PopupMenuItem(
        ...
        onTap: ()async{
            ...
            upcomingProjects.removeWhere((element) => element['title'] == title);
            canceledProjects.removeWhere((element) => element['title'] == title);
            ongoingProjects.removeWhere((element) => element['title'] == title);
            completedProjects.removeWhere((element) => element['title'] == title);
            setState(() {
  ...
  void bottomSheet(dynamic deviceSize){
    showModalBottomSheet(elevation:5,
        ...
        context: context,backgroundColor: Colors.grey[350], builder: (BuildContext context){
          return Padding(
            padding: const EdgeInsets.all(18.0),
            child: SizedBox(height: deviceSize.height*0.13,width: 50,
              child: Column(mainAxisAlignment: MainAxisAlignment.center,crossAxisAlignment: CrossAxisAlignment.center,
                children: [InkWell(onTap: (){Navigator.of(context).pop();ascendingSort();},
                  child: SizedBox(width: deviceSize.width,height: deviceSize.height*0.05,
                    child: const Center(
                      child:Text("Sort : A-Z",
                  ...
                  InkWell(onTap: ()async{Navigator.of(context).pop();descendingSort();},
                      child: SizedBox(width: deviceSize.width,height: deviceSize.height*0.05,
                        child:const Center(
                          child: Text('Sort : Z-A',style:
